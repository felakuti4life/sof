...
#include <algorithm>
#else
bool WebRtc_AnyOfRangeGreaterThanZero(int* begin, size_t count);
bool WebRtc_AnyOfRangeGreaterThanZero(int* begin, size_t count)
{
    for(int i = 0; i < count; i++)
    {
        if(begin[i] > 0) return true;
    }
...
int WebRtc_ProcessBinarySpectrum(BinaryDelayEstimator* self,
  ...
  const bool non_stationary_farend =
      std::any_of(self->farend->far_bit_counts,
                  ...
                  [](int a) { return a > 0; });
#else
  const bool non_stationary_farend = WebRtc_AnyOfRangeGreaterThanZero(self->farend->far_bit_counts, self->history_size);
#endif
